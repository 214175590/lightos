...
+ function($, win){
    ...
    var root = (function(){
    	var urls = yxosDir.substring(yxosDir.indexOf("//") + 3, yxosDir.length - 1),
    		one = urls.indexOf("/");
    	if(one != -1){
	...
	win.YXOs = {
		...
		control: {			
			...
			// 获得所有窗口
			getWindows: function(){
				return YXOs.desktop.WindowMap.valueSet();
			},
...
+ function($, win, os){
	...
	os.YXWindow = function(ops){
		...
		this.isDrag = ops.isDrag == undefined ? true : ops.isDrag;
		
		this.openFunction = ops.openFunction;
		this.closeFunction = ops.closeFunction;
		this.minFunction = ops.minFunction;
		this.maxFunction = ops.maxFunction;
	...
	os.YXWindow.prototype = {
		...
		open: function() {
			var self = this;
			try{
				if(self.openFunction && Object.prototype.toString.call(self.openFunction) == "[object Function]"){
					self.openFunction.call(os, self);
				}
			}catch(e){}
			if(self.types == 'link'){
		...
		minimize: function(){
			...
			self.body.animate(
			{
				...
				try{
					if(self.minFunction && Object.prototype.toString.call(self.minFunction) == "[object Function]"){
						self.minFunction.call(os, self);
					}
				}catch(e){}
			});
		},
		...
		maximize: function(){
			...
			try{
				if(self.maxFunction && Object.prototype.toString.call(self.maxFunction) == "[object Function]"){
					self.maxFunction.call(os, self);
				}
			}catch(e){}
		},
		...
		close: function(){
			...
			self.body.animate(
			{
				...
				try{
					if(self.closeFunction && Object.prototype.toString.call(self.closeFunction) == "[object Function]"){
						self.closeFunction.call(os, self);
					}
				}catch(e){}
			});
		},
...
+ function($, win, os){
	...
	os.YXTaskBar = function(){
		this.body = $('<div id="yxos-taskbar" class="yxos-taskbar"><div class="taskbar-start" title="开始菜单"><div class="window-start-menu"><i class="icon icon-windows"></i></div></div><div class="taskbar-tab"></div><div class="taskbar-func" title="显示桌面"></div></div>');
	...
	os.YXTaskBar.prototype = {
		...
		bindEvent: function(){
			...
			// 开始菜单点击事件
			self.start_menu.on('click', function(e){
			...
			// 任务栏显示桌面按钮点击事件
			self.taskbar_func.on('click', function(){
				var wins = os.control.getWindows();
				if(wins){
					for(var i = 0, k = wins.length; i < k; i++){
						wins[i].minimize();
					}
				}
			});
...
+ function($, w, os){
	...
	YXDesktop.prototype = {
		...
		setWallpaper: function(paper){
			...
			self.body.css({
				"background-image": YXOs.utils.format("url('{0}')", paper)
			});
			localStorage.setItem("USER_WALLPAPER", paper);
			return this;
		},
		...
		bindEvent: function(){
			...
			self.body.on('click', function(e){
				...
				os.callEvent("DesktopClick", self.body);
				os.utils.stopEvent(e);
			});
...
+ function($, win, os){
	...
	os.YXWindowHandle = function(options){
		...
		this.icon = os.utils.getOptionValue(options, 'icon', os.path + 'content/images/icons/i01.png', 'String');
		// 状态
...
+ function($, win, os){
	...
	os.YXDesktopIcon = function(options){
		...
		this.openFunction = options.openFunction;
		this.closeFunction = options.closeFunction;
		this.minFunction = options.minFunction;
		this.maxFunction = options.maxFunction;
		...
		openWindow: function(){
			...
			if(self.window === null){
				self.window = new YXOs.YXWindow({
					...
					needClose: self.needClose,
					needMin: self.needMinimize,
					needMax: self.needMaximize,
					openFunction: self.openFunction,
					closeFunction: self.closeFunction,
					minFunction: self.minFunction,
					maxFunction: self.maxFunction,
					isDrag: self.isDrag,
