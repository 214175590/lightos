...
(function(){
    ...
    Modal.prototype.hideModal = function() {
        var that = this
        if(that.$element.hasClass('custom')){
        	this.$element.remove();
        } else {
        	this.$element.hide();
        }
    ...
    ModalTrigger.prototype.show = function(option) {
        ...
        if(options.type === 'custom' && custom) {
            ...
            var onLoadBroken = function() {
                ...
                if(brokenContent) {
                	$body.html( typeof(brokenContent)=='object' ? '加载超时，请检查网络或联系管理员.' : brokenContent);
                	console && console.error("页面加载超时：",brokenContent);
                } else {
                	$body.html('加载超时，请检查网络或联系管理员.');
                }
            };
